...
function dfa = K2dfa(kset)
    ...
    IandC = I;
    for i = 1:length(C)
        if isequal(ismember([C(i, 1)], I), [1]) == 0
            IandC{length(IandC)+1, 1} = C(i, 1);
        end
    end
    ...
    for i = 1:length(IandC)
        ...
        for len = 1:length(str)
            if isequal(ismember([str(1:len)], SetOfStates), [1]) == 0
                SetOfStates{length(SetOfStates)+1, 1} = str(1:len);
            end
        end
    end
    ...
    for i = 1:length(T)
        ...
        if length(str) > 1
            ...
            if isequal(ismember([t_suff], SetOfStates), [1]) == 0
                SetOfStates{length(SetOfStates)+1, 1} = t_suff;
            end
            if isequal(ismember([t_pref], SetOfStates), [1]) == 0
                SetOfStates{length(SetOfStates)+1, 1} = t_preff;
            end
        else break;
        end 
    end
    ...
    for i = 1:length(F)
        ...
        FinalAcceptStates(length(FinalAcceptStates)+1, 1) = index;
    end
    ...
    % return DFA %
    dfa = DFA(SetOfStates, Alphabets, TransitionMatrix, InitialState, FinalAcceptStates, FinalRejectStates, RED, BLUE);
