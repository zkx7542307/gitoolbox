...
%        lcs The longest common string of output starting from state q
% Output: An onward PTT

function [ptt, q, f] = Onward_PTT(ptt, q, lcs)
...
%   Detailed explanation goes here
    ...
    for i = 1:length(iAlphabet)
        ...
        if ptt.StateTransition(q, i) ~= 0
            [ptt, q_next, w] = Onward_PTT(ptt, ptt.StateTransition(q, i), alpha);
            if w == 0 
                continue;
            end
            if ~strcmp(w, '')
